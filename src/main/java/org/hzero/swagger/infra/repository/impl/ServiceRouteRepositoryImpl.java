...
public class ServiceRouteRepositoryImpl extends BaseRepositoryImpl<ServiceRoute> implements ServiceRouteRepository {
    ...
    public MultiKeyMap getAllRunningInstances() {
        ...
        Map<String, ServiceRoute> serviceIdRouteMap = routeList.stream()
                .collect(Collectors.toMap(ServiceRoute::getServiceCode, route -> route));
        ...
        for (String serviceId : serviceIds) {
            for (ServiceInstance instance : discoveryClient.getInstances(serviceId)) {
                ...
                if (multiKeyMap.get(serviceId, version) == null) {
                    ServiceRoute route = serviceIdRouteMap.get(serviceId);
                    if (route == null) {
                        continue;
                    }
                    multiKeyMap.put(serviceId, version, route);
                }
            }
        }
        return multiKeyMap;
